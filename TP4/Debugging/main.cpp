...
double a; // contient la valeur a dont on veut calculer sqrt(a)
    int x = 0; // contient x
    int x2 = 0; // contient le terme suivant x dans la suite
...
int main()
{
    ...
    while(a <= 0)
    {
        ...
        cin >> a;
    }  // On ne peut calculer la racine carree que d'un nombre positif

    cout << "Quel precision voulez-vous choisir ? "; // Il faut changer les guillemets ICI.
    cin >> precision;

    cout.precision(precision); // AJouter un ; et changer 15 par la variable precision
